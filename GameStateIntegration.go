...
func GamestateIntegration(config *Config) {
	...
	if !fileExists(path.Join(config.Path, "cfg", "gamestate_integration_spddl.cfg")) { // TODO path.Join()
...
func createGamestateIntegrationTemplate(csgoPath string) {
	...
	err := ioutil.WriteFile(path.Join(csgoPath, "cfg", "gamestate_integration_spddl.cfg"), data, 0644)
	if err != nil {
		log.Println(err)
	}
}
